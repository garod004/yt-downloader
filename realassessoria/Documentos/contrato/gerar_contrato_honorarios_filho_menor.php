...
$filho_menor = null;
$sql_filho = "SELECT nome, cpf, data_nascimento FROM filhos_menores WHERE cliente_id = ? ORDER BY id DESC LIMIT 1";
if ($stmt_filho = $conn->prepare($sql_filho)) {
    $stmt_filho->bind_param('i', $cliente_id);
    $stmt_filho->execute();
    $result_filho = stmt_get_result($stmt_filho);
    if ($result_filho->num_rows === 1) {
        $filho_menor = $result_filho->fetch_assoc();
    }
    $stmt_filho->close();
}
...
$filho_nome = 'Nao Informado';
$filho_cpf = 'Nao Informado';
$filho_data_nasc = 'Nao Informada';
if ($filho_menor) {
    ...
    $filho_data_nasc_raw = $filho_menor['data_nascimento'] ?: '';
    if ($filho_data_nasc_raw !== '' && strlen($filho_data_nasc_raw) === 10 && strpos($filho_data_nasc_raw, '-') !== false) {
        $filho_data_nasc = htmlspecialchars(implode('/', array_reverse(explode('-', $filho_data_nasc_raw))));
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Contrato de Prestacao de Servico - Filho Menor</title>
    <style>
        @page {
            size: A4;
            margin: 4mm 20mm;
...
<body>
    ...
    <div class="info-row">
        ...
        na qualidade de representante legal do(a) filho(a) menor <strong>$filho_nome</strong>, inscrito(a) no CPF sob o no $filho_cpf,
        nascido(a) em $filho_data_nasc.
        </div>
    </div>
    ...
    <div class="content-box">
        ...
        <p>CLAUSULA 4ª - Caso haja morte ou incapacidade civil em ocorrencia do contratado, sua advogada constituida como representante legal recebera os honorarios na proporcao do trabalho realizado.</p>
        <p>CLAUSULA 5ª - As partes elegem o foro da Comarca de Manaus - AM.</p>
        <p>CLAUSULA 6ª - Por estarem assim justos e contratados, firmam o presente CONTRATO DE HONORARIOS POR SERVICOS PRESTADOS em duas vias de igual teor e forma, para que o mesmo, nos seus devidos fins de direito, produza seus juridicos e legais efeitos, juntamente com as 02 (duas) testemunhas, igualmente subscritas, identificadas e assinadas.</p>
    </div>
...
try {
    ...
    $nome_arquivo = 'Contrato_Honorarios_Padrao_Filho_Menor_' . str_replace(' ', '_', $nome_cliente) . '.pdf';
    $dompdf->stream($nome_arquivo, array('Attachment' => 1));
} catch (Throwable $e) {
    error_log('Erro ao gerar contrato filho menor em PDF (Dompdf): ' . $e->getMessage());
    die('Erro ao gerar contrato filho menor em PDF: ' . $e->getMessage());
}
