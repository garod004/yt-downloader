...
if ($stmt_incapaz = $conn->prepare($sql_incapaz)) {
    ...
    $result_incapaz = stmt_get_result($stmt_incapaz);
    if ($result_incapaz->num_rows === 1) {
        $incapaz = $result_incapaz->fetch_assoc();
    }
    $stmt_incapaz->close();
}
...
<head>
    <meta charset="UTF-8">
    <title>Contrato de Prestacao de Servico - Incapaz</title>
    <style>
        @page {
            size: A4;
            margin: 4mm 20mm;
...
<body>
    ...
    <div class="info-row">
        ...
        na qualidade de representante legal do(a) incapaz <strong>$incapaz_nome</strong>, inscrito(a) no CPF sob o no $incapaz_cpf,
        nascido(a) em $incapaz_data_nasc.
        </div>
    </div>
...
try {
    ...
    $nome_arquivo = 'Contrato_Honorarios_Padrao_Incapaz_' . str_replace(' ', '_', $nome_cliente) . '.pdf';
    $dompdf->stream($nome_arquivo, array('Attachment' => 1));
} catch (Throwable $e) {
    error_log('Erro ao gerar contrato incapaz em PDF (Dompdf): ' . $e->getMessage());
    die('Erro ao gerar contrato incapaz em PDF: ' . $e->getMessage());
}
