...
// 4. Buscar dados do incapaz vinculado ao cliente
$conn->query("CREATE TABLE IF NOT EXISTS incapazes (
...
if ($stmt_incapaz = $conn->prepare($sql_incapaz)) {
    ...
    $incapaz = stmt_fetch_first_assoc_contrato_justica($stmt_incapaz);
    $stmt_incapaz->close();
}
...
$incapaz_nome = 'Não Informado';
$incapaz_cpf = 'Não Informado';
$incapaz_data_nasc = 'Não Informada';
if ($incapaz) {
    $incapaz_nome = htmlspecialchars(mb_convert_case($incapaz['nome'] ?: 'Não Informado', MB_CASE_TITLE, 'UTF-8'));
    $incapaz_cpf = htmlspecialchars($incapaz['cpf'] ?: 'Não Informado');
...
<body>
    ...
    <div class="bloco">
        ...
        de identidade nº $rg_cliente, inscrito(a) no CPF/MF sob o nº $cpf_cliente, residente e domiciliado(a) em $endereco_cliente, $cidade_cliente - $uf_cliente, doravante denominado(a) <strong>CONTRATANTE</strong>. neste ato,
