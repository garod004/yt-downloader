...
function stmt_fetch_first_assoc_contrato_trintaporcento_justica($stmt) {
...
if ($stmt = $conn->prepare($sql)) {
    ...
    $stmt->execute();
    $cliente = stmt_fetch_first_assoc_contrato_trintaporcento_justica($stmt);
    $stmt->close();
} else {
...
if ($stmt_filho = $conn->prepare($sql_filho)) {
    ...
    $stmt_filho->execute();
    $filho_menor = stmt_fetch_first_assoc_contrato_trintaporcento_justica($stmt_filho);
    $stmt_filho->close();
}
...
<body>
    ...
    <div class="bloco">
        ...
        de identidade nº $rg_cliente, inscrito(a) no CPF/MF sob o nº $cpf_cliente, residente e domiciliado(a) em $endereco_cliente, $cidade_cliente - $uf_cliente, doravante denominado(a) <strong>CONTRATANTE</strong>.
    </div>
    ...
    <div class="bloco">
        ...
        valor de <strong>30% (Trinta por cento)</strong> no ato da implantação do benefício. Caso
...
$nome_arquivo = 'Contrato_Justica_30_' . str_replace(' ', '_', $nome_cliente) . '.pdf';
