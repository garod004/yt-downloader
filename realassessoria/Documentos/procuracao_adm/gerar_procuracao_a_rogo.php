...
// Garante tabela a_rogo para evitar falha em ambientes ainda nao migrados.
...
if ($stmt_a_rogo = $conn->prepare($sql_a_rogo)) {
    ...
    $stmt_a_rogo->execute();
    $a_rogo = stmt_fetch_first_assoc_compat_incapaz($stmt_a_rogo);
    $stmt_a_rogo->close();
}
...
try {
    ...
    error_log('Erro ao gerar procuracao a rogo em PDF (Dompdf): ' . $e->getMessage());
    die('Erro ao gerar procuracao a rogo em PDF: ' . $e->getMessage());
}
