...
if ($stmt_filho_menor = $conn->prepare($sql_filho_menor)) {
    ...
    $filho_menor = stmt_fetch_first_assoc_compat_incapaz($stmt_filho_menor);
...
if ($filho_menor) {
    ...
    $filho_menor_data_nasc_raw = $filho_menor['data_nascimento'] ?: '';
    if ($filho_menor_data_nasc_raw !== '' && strlen($filho_menor_data_nasc_raw) === 10 && strpos($filho_menor_data_nasc_raw, '-') !== false) {
        $filho_menor_data_nasc = htmlspecialchars(implode('/', array_reverse(explode('-', $filho_menor_data_nasc_raw))));
    }
}
...
<body>
    <div class="documento">
        ...
        <div class="content">
            ...
            neste ato, na qualidade de representante legal do(a) filho(a) menor <strong>$filho_menor_nome</strong>,
            inscrito(a) no CPF sob o nº $filho_menor_cpf, nascido(a) em $filho_menor_data_nasc.
        </div>

        <div class="section-title">OUTORGADOS:</div>
        <div class="content">
            <strong>Real Assessoria Previdenciaria</strong>, CNPJ: 13.244.474/0001-20, Rua M, nº 65, Nova Cidade,
