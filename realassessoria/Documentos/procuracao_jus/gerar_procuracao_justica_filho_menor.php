...
$filho_menor = null;
$sql_filho_menor = 'SELECT nome, cpf, data_nascimento FROM filhos_menores WHERE cliente_id = ? ORDER BY id DESC LIMIT 1';

if ($stmt_filho_menor = $conn->prepare($sql_filho_menor)) {
    $stmt_filho_menor->bind_param('i', $cliente_id);
    $stmt_filho_menor->execute();
    $filho_menor = stmt_fetch_first_assoc_compat($stmt_filho_menor);
    $stmt_filho_menor->close();
}
...
$filho_menor_nome = 'Nao Informado';
$filho_menor_cpf = 'Nao Informado';
$filho_menor_data_nasc = 'Nao Informada';
if ($filho_menor) {
    $filho_menor_nome = htmlspecialchars(mb_convert_case($filho_menor['nome'] ?: 'Nao Informado', MB_CASE_TITLE, 'UTF-8'));
    $filho_menor_cpf = htmlspecialchars($filho_menor['cpf'] ?: 'Nao Informado');
    $filho_menor_data_nasc = htmlspecialchars(formatar_data_brasil_compat($filho_menor['data_nascimento'] ?: ''));
}
...
<head>
    <meta charset="UTF-8">
    <title>Procuração Justiça Filho Menor</title>
    <style>
        @page {
            margin: 15mm 20mm;
        }
...
<body>
    ...
    <div class="content">
        ...
        telefone: {$telefone_cliente}, e-mail: {$email_cliente}, neste ato na qualidade de representante legal do(a)
        filho(a) menor <strong>{$filho_menor_nome}</strong>, inscrito(a) no CPF sob o nº {$filho_menor_cpf}, nascido(a) em {$filho_menor_data_nasc}.
    </div>
...
$dompdf->render();
$dompdf->stream('procuracao_justica_filho_menor.pdf', array('Attachment' => false));
exit;
